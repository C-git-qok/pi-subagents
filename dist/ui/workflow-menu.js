 ...
 */
import { pauseWorkflowTask, resumeWorkflowTask } from "../workflow/task.js";
 ...
 */
export async function showWorkflowDialog(ctx, task, deps) {
    ...
    () => ({
        ...
        onOpenAgent: recordId => {
            ...
            void deps.viewAgentConversation(ctx, record)
                .catch(err => ctx.ui.notify(`Could not open the conversation: ${err instanceof Error ? err.message : String(err)}`, "warning"))
 ...
 * the reader at `main`.
 */
export function openWorkflowFromFleet(id, deps) {
...
export async function showWorkflowsMenu(ctx, deps) {
