...
import { buildPhaseGroups, collapse, formatDuration, header, sizeWarning, stats, } from "../workflow/progress.js";
/**
 ...
 * play.
 */
export function formatModel(entry, opts) {
 ...
 */
export function formatThinking(entry) {
 ...
 */
export function agentStatSegments(entry) {
    const parts = [];
    if (entry.agentType)
 ...
 */
export function layoutWorkflowCard(input) {
    ...
    const labelColumn = Math.min(LABEL_COLUMN_MAX, Math.max(0, ...groups.flatMap(group => group.agents.map(a => visibleWidth(a.label)))));
    groups.forEach((group, groupIndex) => {
        ...
        lines.push(clampLine([
            ...
            {
                text: `${lastGroup ? glyphs.groupBottom
    ...
    // ---- Size warning ----
    const totalTokens = input.totalTokens ?? agents.reduce((sum, entry) => sum + (entry.tokens ?? 0), 0);
...
export function renderWorkflowCard(input, theme) {
 ...
 */
export function renderWorkflowEntryCard(data, theme) {
    if (!data)
        return undefined;
