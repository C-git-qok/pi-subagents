...
import { buildPhaseGroups, displayState, formatDuration, header, isLive, } from "../workflow/progress.js";
import { SPINNER } from "./agent-widget.js";
import { ASCII_GLYPHS, clampLine, formatCompactTokens, formatModel, formatThinking, REPLAYED_ANNOTATION, styleWorkflowCardLines, UNICODE_GLYPHS, } from "./workflow-card.js";
...
export const UNICODE_DIALOG_GLYPHS = {
...
export const ASCII_DIALOG_GLYPHS = {
 ...
 */
export function dialogRowGlyph(state, glyphs, spinnerFrame = 0) {
    switch (state) {
        case "done":
...
export const WORKFLOW_DIALOG_FILTERS = [
    "all",
    "running",
    "queued",
    "done",
...
export function initialWorkflowDialogState(initialPhaseIndex = 0) {
    return {
...
export function workflowDialogContentWidth(terminalWidth) {
    return Math.max(12, terminalWidth - 6);
}
const clampIndex = (index, length) => length === 0 ? 0 : Math.min(Math.max(0, Math.trunc(index)), length - 1);
/**
 ...
 */
export function resolveWorkflowDialog(input) {
    ...
    const visibleAgents = input.state.filter === "all"
        ? [...all]
 ...
 */
export function subStatusAnnotations(entry, state, now) {
    const parts = [];
    if (entry.isolation)
...
const lineWidth = (line) => line.reduce((sum, s) => sum + visibleWidth(s.text), 0);
 ...
 */
export function leftPaneWidth(width) {
...
const previewLines = (preview) => (preview ? preview.split("\n") : []);
 ...
 */
export function agentActions(entry, workflowActive) {
...
const detailBody = (text, width) => clampLine([{ text: `   ${text}`, color: "dim" }], width);
...
function agentRow(options) {
    ...
    const duration = entry.durationMs ? [{ text: `${formatDuration(entry.durationMs)} `, color: "dim" }] : [];
 ...
 * different sizes.
 */
export function layoutWorkflowDialog(input) {
    ...
    for (let i = phases.start; i < Math.min(phases.end, view.groups.length); i++) {
        ...
        const glyph = group.status === "done" ? glyphs.tick
    ...
    if (!inPhases && entry) {
        ...
        if (entry.toolCalls)
            stats.push(`${entry.toolCalls} tool call${entry.toolCalls === 1 ? "" : "s"}`);
        if (entry.durationMs)
    ...
    const agentPaneTitle = state.filter === "all" ?
 ...
 * Keys
 * ------------------------------------------------------------------------- */
const nextFilter = (filter) => WORKFLOW_DIALOG_FILTERS[(WORKFLOW_DIALOG_FILTERS.indexOf(filter) + 1) % WORKFLOW_DIALOG_FILTERS.length];
/**
 ...
 */
export function handleWorkflowDialogKey(data, state, view) {
...
export function plainWorkflowDialogLines(lines) {
