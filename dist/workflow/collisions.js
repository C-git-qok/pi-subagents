 ...
 */
export const FOREIGN_WORKFLOW_TOOL_NAMES = new Set([
...
export function decideWorkflowCollision(input) {
    ...
    if (input.pinned) {
        ...
        return {
            kind: "report",
            message: `Another extension (${source}) already registers a "${SUBAGENT_TOOL_NAMES.WORKFLOW}" tool. ` +
    ...
    return {
        kind: "standDown",
        message: `Another extension (${source}) already provides a "${foreign.name}" tool, so this extension's ` +
