...
export function savedWorkflowRoots(cwd) {
    return [
 ...
 */
export function readSavedWorkflow(name, cwd) {
    const trimmed = name.trim();
    if (isUnsafeName(trimmed)) {
        return {
            ok: false,
            message: `"${name}" is not a usable workflow name. Use letters, digits, dots, hyphens and underscores only ` +
    ...
    for (const root of roots) {
        if (isSymlink(root))
            continue; // reject a symlinked root entirely, as skill-loader does
        ...
        if (!hasMetaDeclaration(script)) {
            return {
                ok: false,
                message: `"${path}" is not a workflow script — it has no \`export const meta = { name, description }\` ` +
    ...
    return {
        ok: false,
        message: `No saved workflow named "${trimmed}". Looked in: ${roots.join(", ")}. ` +
            (known.length > 0
 ...
 */
export function resolveWorkflowSource(ref, cwd) {
...
export function listSavedWorkflows(cwd) {
    const names = new Set();
    for (const root of savedWorkflowRoots(cwd)) {
        ...
        for (const entry of entries) {
            ...
            if (isUnsafeName(name))
                continue;
            if (isWorkflowFile(join(root, entry)))
                names.add(name);
        }
    }
    return [...names].sort();
...
function isWorkflowFile(path) {
    try {
        if (statSync(path).size > MAX_SCRIPT_LENGTH)
            return false;
    }
    catch {
        return false;
 ...
 */
export function resolveWorkflowScript(params, cwd) {
    ...
    return {
        ok: false,
        message: "Provide `script` (inline source), `scriptPath` (a file to read), or `name` (a saved workflow). " +
